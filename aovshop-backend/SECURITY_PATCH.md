...
2. Backend dùng `npm start` sẽ chạy migration bảo mật sau bước đồng bộ schema hiện có. Nếu chạy trực tiếp `node dist/index.js` hoặc dùng chế độ dev trên database cũ, chạy `npm run db:migrate-security` trước khi phục vụ request bằng bản mới. Migration đọc `.env` và `.env.local`, vì vậy cần chọn đúng môi trường.
...
Chạy `npm test` trong backend: 18 kiểm thử hồi quy dùng HTTP local và SQLite tạm, có kiểm tra request đồng thời, hoàn tác khi ghi sổ lỗi, thu hồi JWT, liên kết Google và migration chạy lặp. Không dùng database thật hoặc gửi giao dịch thanh toán thật.
